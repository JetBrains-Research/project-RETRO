...
from retro_pytorch.train_functions import (
    aggregate_batches,
    calc_loss,
    calc_loss_concat,
    grad_step,
...
for train_steps, (seq, docs) in enumerate(tqdm(train_dl, total=total_items // batch_size), start=1):
    ...
    loss = calc_loss_concat(seq, docs, retro, fetch_neighbours)
    ...
    if train_steps % freq_val == 0:
        ...
        losses_val_cur = val_steps_concat(retro, no_retrieve, fetch_neighbours, aggregate)
        losses_val_rnd_cur = val_steps_concat(retro, no_retrieve, fetch_random_chunk, aggregate)
        losses_val_pure_rnd_cur = val_steps_concat(retro, no_retrieve, generate_pure_random_chunk, aggregate)
        losses_ideal = val_steps_concat(retro, no_retrieve, fetch_ideal, aggregate)

        max_val_loss, saved_ind, saved_last_ind, val_dl_iter = val_update(
            retro,
            losses_val,
            [losses_val_cur, losses_val_rnd_cur, losses_val_pure_rnd_cur, losses_ideal],
            paths.model_folder,
            model_name,
            val_dl_iter,
            f_val,
